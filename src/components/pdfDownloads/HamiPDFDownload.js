...
const HamiPDFDownload = () => {
  ...
  //     const response = await axios.get(`/hami-reports/${selectedMonth}/${selectedYear}/pdf`, {
  //       responseType: 'blob'
  //     });
  ...
  //     link.href = url;
  //     link.download = `hami-report-${selectedMonth}-${selectedYear}.pdf`;
...
// console.log('Response data length:', response.data.byteLength || response.data.length);
...
// for mobile, add permissions in xml for iphone and android to allow read write
const handleDownload = async () => {
  setDownloading(true);
  ...
  try {
    ...
    if (!Capacitor.isNativePlatform()) {
      // showToast('PDF download is only available on mobile app', 'error');
      // setDownloading(false);
      // return;
  ...
  console.log('Response data length:', response.data.byteLength || response.data.length);
    ...
    const response = await axios.get(
      `/hami-reports/${selectedMonth}/${selectedYear}/pdf`,
      { responseType: 'arraybuffer' }
    );
    ...
    const fileName = `hami-report-${selectedMonth}-${selectedYear}.pdf`;
    ...
    showToast('PDF downloaded and opened successfully!');
    setTimeout(() => hideToast(), 3000);
    ...
    if (error.message.includes('Permission')) {
      showToast('Storage permission denied. Please enable it in settings.', 'error');
    } else if (error.message.includes('No app')) {
      showToast('No PDF viewer app found. Please install one.', 'error');
    } else {
      showToast('Error downloading PDF. Please try again.', 'error');
    }
  } finally {
  ...
  const getCompletionStats = () => {
    ...
    const namazDays = days.filter(day => day.namaz === 'yes').length;
    const hifzDays = days.filter(day => day.hifz === 'yes').length;
    const nazraDays = days.filter(day => day.nazra === 'yes').length;
    const tafseerDays = days.filter(day => day.tafseer === 'yes').length;
    const hadeesDays = days.filter(day => day.hadees === 'yes').length;
...
export default HamiPDFDownload;
