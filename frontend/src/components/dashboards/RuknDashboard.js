...
const RuknDashboard = () => {
  ...
  const fetchDays = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/rukn-reports/${selectedMonth}/${selectedYear}/days`);
      if (response.data && response.data.success) {
        const fetchedDays = response.data.days || [];
        setDays(fetchedDays);
        
        // Calculate stats for Rukn
        const totalDays = fetchedDays.length;
        // For Rukn, consider a day completed if namaz is yes and at least one other activity is yes
        const completedDays = fetchedDays.filter(day => {
          if (day.namazBarwaqtAdaigi !== 'yes') return false;
          return day.nazraQuran === 'yes' || day.hifzQuran === 'yes' || 
                 day.mutaleyaTafseer === 'yes' || day.mutaleyaHadees === 'yes' ||
                 day.mutaleyaLiterature === 'yes';
        }).length;
        
        const namazDays = fetchedDays.filter(day => day.namazBarwaqtAdaigi === 'yes').length;
        const hifzDays = fetchedDays.filter(day => day.hifzQuran === 'yes').length;
        const nazraDays = fetchedDays.filter(day => day.nazraQuran === 'yes').length;
        const tafseerDays = fetchedDays.filter(day => day.mutaleyaTafseer === 'yes').length;
  ...
  const getTodayProgress = () => {
    ...
    return {
      namaz: todayData.namazBarwaqtAdaigi === 'yes',
      hifz: todayData.hifzQuran === 'yes',
      nazra: todayData.nazraQuran === 'yes',
      tafseer: todayData.mutaleyaTafseer === 'yes',
      hadees: todayData.mutaleyaHadees === 'yes'
    };
  };
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <p className="text-sm text-gray-500 mt-2">
          Current Report: {selectedMonth} {selectedYear}
        </p>
      </div>
