...
const UmeedwarDailyReport = () => {
  ...
  const fetchDayData = async () => {
    try {
      ...
      const response = await axios.get(`/umeedwar-reports/day/${selectedMonth}/${selectedYear}/${selectedDate}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Build dayData object with all Umeedwar fields
      const dayData = {
        date: selectedDate,
        month: selectedMonth,
        year: selectedYear
      ...
      umeedwarFields.forEach(field => {
        dayData[field] = selectedDayData[field] || 'no';
      });

      const response = await axios.post(`/umeedwar-reports/${selectedMonth}/${selectedYear}/${selectedDate}`, dayData);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex flex-wrap items-center gap-2 sm:space-x-3">
              ...
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium border bg-yellow-100 text-yellow-800 border-yellow-300">
                Umeedwar Rukn
              </span>
            </h1>
          ...
          <div className="space-y-6">
            ...
            <div className="border-b border-gray-200 pb-4">
              <label className="text-lg font-medium text-gray-900 mb-3 block">
                {fieldLabels[umeedwarFields[0]] || umeedwarFields[0]}
              </label>
              <div className="flex space-x-4">
                <button
                  onClick={() => handleFieldChange(umeedwarFields[0], 'yes')}
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md border ${
                    getFieldValue(umeedwarFields[0]) === 'yes'
                ...
                <button
                  onClick={() => handleFieldChange(umeedwarFields[0], 'no')}
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md border ${
                    getFieldValue(umeedwarFields[0]) === 'no'
            ...
            {/* Umeedwar Fields - Grid Layout */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {umeedwarFields.slice(1).map((fieldName) => (
...
export default UmeedwarDailyReport;
